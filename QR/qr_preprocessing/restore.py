...
def draw_points(img, cnt, epsilon, color):
    ...
    outter_point = np.array([0,0])

    if color == GREEN: # Green dot
        for point in approx:

            if np.all(point >= outter_point):
                outter_point = point[0]

            point = cv2.circle(img, (point[0][0], point[0][1]), 3, color, -1)
...
for cnt in contours_none:
    ...
    if is_circle(cnt):
        ...
        # 원형이 아닐 경우 초록색으로 그리기
        point_val = draw_points(none_img, cnt, 0.1, GREEN)

        if np.all(point_val >= max_point):
            max_point = point_val
