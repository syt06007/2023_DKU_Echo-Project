...
# Load an image
frame = cv.imread("qr_preprocessing\qr_img\cap2.PNG")
# frame = cv.imread("test.jpg")

qcd = cv.QRCodeDetector()

data, box, straight_qrcode = qcd.detectAndDecode(frame)
print('decode :: ', data)
print()
...
classes = open('yolo/qrcode.names').read().strip().split('\n')
net = cv.dnn.readNetFromDarknet('yolo/qrcode-yolov3-tiny.cfg', 'yolo/qrcode-yolov3-tiny.weights')
