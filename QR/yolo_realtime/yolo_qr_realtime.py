...
classes = open("yolo_realtime/qrcode.names").read().strip().split('\n')
net = cv.dnn.readNetFromDarknet('yolo_realtime/qrcode-yolov3-tiny.cfg', 'yolo_realtime/qrcode-yolov3-tiny.weights')
...
def postprocess(frame, outs):
    ...
    for i in indices:
        ...
        cropped_image = frame[top-50:top + height+50, left-50:left + width+50]
        try:
            cv.imshow("crop", cropped_image)
            cv.imwrite("cropped.jpg", cropped_image)
            retval, decoded_info, points, straight_qrcode = qcd.detectAndDecodeMulti(cropped_image)
            print("retval:{0}, dec:{1}".format(retval, decoded_info))
            try: 
                url = decoded_info
                print(url[0])
                response = urllib.request.urlopen(url[0]).read()
